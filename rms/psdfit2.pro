pro psdfit2, basedir, binp=pbin, noupdate=noupdate, manual=manual, nulor=lorn
...
save,basedir,pbin,lorn,filename='restidl.sav' ;save these so that they are not overwritten
...
xdrfu_r1,base1+base2+'0004096_signormpsd_corr.xdrfu',f128s,p128s
xdrfu_r1,base1+base2+'0004096_errnormpsd_corr.xdrfu',f128s,p128se
...
PLOTERROR,ff,pf,pfe,/xlog,/ylog,psym=10,/nohat,xrange=[min(ff),max(ff)],/xstyle,position=[0.15,0.4,0.98,0.98],xtitle='Frequency',yrange=[min(pf(where(pf gt 0.))),max(pf)*2.],/ystyle,color=255
...
FOR i=0, 3*nlor-1, 3 DO BEGIN
    OPLOT,ff,f_lor(ff,s[i:i+2]),thick=2,linestyle=2,color=255
    pow=pow+f_lor(ff,s[i:i+2])
ENDFOR
OPLOT,ff,pow,thick=3,color=255
pow=0
...
WHILE (1) DO BEGIN
    ...
    PLOTERROR,ff,pf,pfe,/xlog,/ylog,psym=10,/nohat,xrange=[min(ff),max(ff)],/xstyle,yrange=[min(pf(where(pf gt 0.))),max(pf)*2.],color=255
    FOR i=0, 3*nlor-1, 3 DO BEGIN
        OPLOT,ff,f_lor(ff,s[i:i+2]),thick=2,linestyle=2,color=255
    pow=pow+f_lor(ff,s[i:i+2])
    ENDFOR 
...
!p.multi=[0,1,2]
PLOTERROR,ff,pf*ff,pfe*ff,/xlog,/ylog,psym=10,/nohat,xrange=[min(ff),max(ff)],/xstyle,position=[0.15,0.4,0.98,0.98],xtickname=REPLICATE(' ', 6),yrange=[1e-5,max(pf*ff)*2.],/ystyle,color=255
FOR i=0, 3*nlor-1, 3 DO BEGIN
        OPLOT,ff,f_lor(ff,r[i:i+2])*ff,thick=2,linestyle=2,color=255
        pow=pow+f_lor(ff,r[i:i+2])
    ENDFOR
OPLOT,ff,pow*ff,thick=2,color=255
Plot,ff,((pf-pow)/pfe)^2.*ff,/xlog,xrange=[min(ff),max(ff)],/xstyle,psym=10,position=[0.15,0.1,0.98,0.4],xtitle='Frequency',color=255;,yrange=[1e-7,1e-1]
!p.multi=0
