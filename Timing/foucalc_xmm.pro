...
;          dpath    : string containing the path to the observation
...
;                                        nof undefined
;              
;              dtlevhf        : Detection level at high frequency 
; 
;
;       -- for the deadtime correction of the psds (kw3)
;              xmm_dt         : lightcurve dt for deadtime correction
;  
;
;                                          
...
;
; EXAMPLE:
;          see xmm_fourier.pro for several examples 
;
;
...
;				   is added/
;
;          Version 1.19, 2017/06/29 Ozan Toyran
;                                  Removed HEXTE related codes for our
;                                  work with XMM
;                                  zhang_dt is now xmm_dt
...
;;
;;
IF (n_elements(xmm_dt) NE 0) THEN BEGIN    
    basic_history(nbasic-14)='Keyword xmm_dt (foucalc)='+string(xmm_dt)
ENDIF ELSE BEGIN 
    basic_history(nbasic-14)='Keyword xmm_dt has not been set (foucalc)'
ENDELSE 
        ...
        IF (n_elements(avg_bkg) NE 0) THEN BEGIN
            psdnorm,mean(rate(*,chan)),length,dseg,pd, $
              schlittgen=schlittgen,leahy=leahy,miyamoto=miyamoto, $
                    avgback=avg_bkg[chan],chatty=chatty
            bkg=avg(pd)
            print, 'Average background psd is :' + strtrim(bkg[0],1)
...
FOR chan=0,nch-1 DO BEGIN
    ...
    ;;
    ;;
    IF (keyword_set(xmm_dt)) AND (keyword_set(chatty)) THEN BEGIN 
       print,'foucalc: XMM deadtime correction is applied'
    ENDIF  
    ;;
    ;; 
    IF (keyword_set(xmm_dt)) THEN BEGIN 
        totrate=mean(rate(*,chan))
        xmm_psdcorr,totrate,length,dseg, $
          cfreq,noipd, $
                    unnormalized=1, dtlevhf=hfdtlev,dpath=dirname, $
          chatty=chatty
        ...
        IF (n_elements(avg_bkg) NE 0) THEN BEGIN
            xmm_psdcorr,totrate,length,dseg, $
              cfreq,noinormpd,  $
              schlittgen=schlittgen,leahy=leahy,miyamoto=miyamoto, $
              avgback=avg_bkg[chan], dtlevhf=hfdtlev,dpath=dirname, $
              chatty=chatty
        ENDIF ELSE BEGIN 
            xmm_psdcorr,totrate,length,dseg, $
              cfreq,noinormpd,dpath=dirname,dtlevhf=hfdtlev, $
              schlittgen=schlittgen,leahy=leahy,miyamoto=miyamoto, $
              chatty=chatty
        ENDELSE 
